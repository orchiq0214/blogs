...
const quizRules = window.CdgaQuizRules;
...
function registerServiceWorker() {
  ...
  window.addEventListener("load", () => {
    navigator.serviceWorker.register("./sw.js").catch(() => {
      // The app remains usable as a normal web page if registration fails.
    });
  });
}
...
function questionPracticeStats() {
  return quizRules.summarizeQuestionPractice({
    questionIds: (appState.bank?.questions || []).map((question) => question.id),
    sessions: appState.state.sessions || [],
  });
}
...
async function loadState() {
  ...
  if (hasGithubConfig()) {
    ...
    if (file) {
      const remote = { ...defaultState(), ...JSON.parse(file.text) };
      appState.state = mergeStates(local, remote);
      appState.githubStateSha = file.sha;
      appState.lastSyncSource = "GitHub";
      saveLocalState();
      if ((local.sessions || []).length || Object.keys(local.wrongQuestions || {}).length) {
        await saveState();
      }
      return;
    }
  }
  appState.state = local;
...
function mergeStates(local, remote) {
  return {
    ...defaultState(),
    ...quizRules.mergeSyncState({ local, remote, updatedAt: nowIso() }),
  };
}
...
async function saveState(options = {}) {
  ...
  try {
    ...
    if (!quizRules.isGithubStateConflict(error.message)) {
...
function activeWrongIds() {
  return quizRules
    .sortWrongQuestionItems(Object.values(appState.state.wrongQuestions || {}))
...
function createSession(options = {}) {
  ...
  const selectedQuestions = ordered.slice(0, count);
  const randomizeOptions = options.randomizeOptions !== false;
  const session = {
    id: makeId("session"),
    status: "in_progress",
    createdAt: nowIso(),
    ...
    optionOrders: Object.fromEntries(
      selectedQuestions.map((question) => [
        question.id,
        quizRules.createOptionOrder(question.options, { randomize: randomizeOptions }),
      ]),
    ),
    report: null,
  };
...
function isQuestionRevealed(session, questionId) {
  return quizRules.isAnswerVisible(session, questionId);
}
...
function decorateQuestion(question, session) {
  ...
  const options = quizRules.applyOptionOrder(question.options, session.optionOrders?.[question.id]);
  const displayAnswer = options.find((option) => option.key === question.answer)?.displayKey || question.answer;
  return {
    ...question,
    options,
    ...
    displayAnswer: revealed ? displayAnswer : null,
...
function updateWrongBook(session, report) {
  ...
  for (const questionId of session.questionIds) {
    ...
    if (item.correctStreak >= 3) item.resolved = true;
  }
}
          ...
          <form class="form-grid" id="startForm">
            ...
            <div class="field">
              ...
              <select class="select" id="revealSelect" name="revealMode">
                <option value="after_submit" selected>整套提交后显示</option>
                <option value="after_each">每题作答后显示</option>
                <option value="manual">手动查看</option>
              </select>
            </div>
            <label class="checkline"><input type="checkbox" name="studyMode" />背题模式：即时显示答案和解析</label>
            <label class="checkline"><input type="checkbox" name="randomOrder" checked />随机出题</label>
            <label class="checkline"><input type="checkbox" name="randomizeOptions" checked />随机选项</label>
...
function renderQuiz() {
  ...
  const isStudy = quizRules.isStudyMode(rawSession);
  const visibleMeta = quizRules.visibleQuestionMeta(current);
  ...
  app.innerHTML = `
    <section class="quiz-toolbar">
      ...
      <div class="actions">
        <button class="btn mode-toggle ${isStudy ? "warning" : ""}" data-action="toggle-study">
          ${isStudy ? "切到考试" : "切到背题"}
        </button>
    ...
    <section class="quiz-layout">
      <article class="panel question-card">
        ...
        <div class="question-meta">
          <span class="pill mode-pill ${isStudy ? "study" : "exam"}">${isStudy ? "背题模式" : "考试模式"}</span>
          <span class="pill">${escapeHtml(current.id)}</span>
          ${visibleMeta.map((label) => `<span class="pill">${escapeHtml(label)}</span>`).join("")}
        </div>
        ...
        ${
          ...
          <div class="explanation">
            <strong>答案：${escapeHtml(current.displayAnswer || current.answer)}</strong>
      ...
      <aside class="panel side-panel">
        <strong>答题卡</strong>
        <div class="subtle">绿色为正确，红色为错误；考试模式提交后显示</div>
        <div class="number-grid">
          ${session.questions
            .map(
              (question, index) => `
            <button class="${quizRules.getAnswerCardClasses({
                questionId: question.id,
                index,
                currentIndex: appState.currentIndex,
                session: rawSession,
              }).join(" ")}" data-action="jump" data-index="${index}">
              ${index + 1}
            </button>
          `,
            )
...
function renderWrongItem(item) {
  ...
  const state = quizRules.wrongQuestionReviewState(item);
  const stateLabel = { pending: "待巩固", high_risk: "高危错题", mastered: "已巩固，仍会复习" }[state];
  return `
    <article class="list-item">
      <h3>${escapeHtml(item.questionId || question.id)} ${escapeHtml(item.title || question.title || "")}</h3>
      <p><strong>${escapeHtml(stateLabel)}</strong> · 累计错 ${escapeHtml(item.count || 0)} 次 · 连续答对 ${escapeHtml(item.correctStreak || 0)} 次</p>
...
function renderWrong() {
  const byId = questionMap();
  const items = quizRules
    .sortWrongQuestionItems(Object.values(appState.state.wrongQuestions || {}))
    .map((item) => ({ ...item, question: byId.get(item.questionId) }));
  const pendingCount = items.filter((item) => quizRules.wrongQuestionReviewState(item) !== "mastered").length;
    ...
    <section class="panel">
      <div class="panel-header">
        <div>
          ...
          <div class="subtle">${pendingCount} 道待巩固 · ${items.length} 道曾错题</div>
        </div>
...
function renderStatistics() {
  const statsByQuestion = questionPracticeStats();
  const rows = (appState.bank?.questions || [])
    .map((question) => ({ question, stats: statsByQuestion[question.id] }))
    .sort((left, right) => {
      if (right.stats.wrong !== left.stats.wrong) return right.stats.wrong - left.stats.wrong;
      if (right.stats.attempts !== left.stats.attempts) return right.stats.attempts - left.stats.attempts;
      return left.question.id.localeCompare(right.question.id);
    });
  const attempts = rows.reduce((total, item) => total + item.stats.attempts, 0);
  const wrong = rows.reduce((total, item) => total + item.stats.wrong, 0);
  const practiced = rows.filter((item) => item.stats.attempts > 0).length;
  const accuracy = attempts ? Math.round(((attempts - wrong) / attempts) * 100) : 0;

  app.innerHTML = `
    <section class="grid">
      <section class="metric-row">
        ${renderMetric(practiced, "已作答题目")}
        ${renderMetric(attempts, "累计作答")}
        ${renderMetric(wrong, "累计错误")}
        ${renderMetric(`${accuracy}%`, "累计正确率")}
      </section>
      <section class="panel">
        <div class="panel-header">
          <div>
            <h1 class="section-title">逐题统计</h1>
            <div class="subtle">按错误次数优先排序；每题在同一套测试中只计一次最终答案。</div>
          </div>
        </div>
        <div class="panel-body stats-list">
          ${
            rows.length
              ? rows
                  .map(({ question, stats }) => {
                    const rate = stats.attempts ? `${Math.round((stats.correct / stats.attempts) * 100)}%` : "-";
                    const recent = stats.lastResult === "correct" ? "最近正确" : stats.lastResult === "wrong" ? "最近错误" : "未作答";
                    return `
                      <article class="stats-row">
                        <div class="stats-question">
                          <strong>${escapeHtml(question.id)} · ${escapeHtml(question.chapter)}</strong>
                          <span>${escapeHtml(question.stem)}</span>
                        </div>
                        <span>作答 ${stats.attempts}</span>
                        <span class="${stats.wrong ? "stat-wrong" : ""}">错 ${stats.wrong}</span>
                        <span>正确率 ${rate}</span>
                        <span class="${stats.lastResult === "wrong" ? "stat-wrong" : ""}">${recent}</span>
                      </article>
                    `;
                  })
                  .join("")
              : `<div class="empty">题库还没有可统计的题目</div>`
          }
        </div>
      </section>
    </section>
...
function render() {
  ...
  if (appState.view === "statistics") renderStatistics();
...
async function startSession(overrides = {}) {
  ...
  const session = createSession({
    ...
    revealMode: form?.revealMode?.value || "after_submit",
    studyMode: form?.studyMode?.checked ?? false,
    randomOrder: form?.randomOrder?.checked ?? true,
    randomizeOptions: form?.randomizeOptions?.checked ?? true,
...
async function toggleStudyMode() {
  const session = appState.activeSession;
  if (!session) return;
  session.settings.studyMode = quizRules.nextStudyMode(session);
  session.updatedAt = nowIso();
  await saveState({ remote: false });
  toast(session.settings.studyMode ? "已切到背题模式" : "已切到考试模式");
  renderQuiz();
}

function mouseTargetIsInteractive(target) {
  return Boolean(target.closest("button, input, select, label, textarea, a"));
}

async function navigateQuestion(direction) {
  if (!appState.activeSession) return;
  const questions = decoratedSession(appState.activeSession).questions;
  const offset = direction === "next" ? 1 : -1;
  const nextIndex = Math.max(0, Math.min(questions.length - 1, appState.currentIndex + offset));
  if (nextIndex === appState.currentIndex) return;
...
document.addEventListener("click", async (event) => {
  ...
  try {
    ...
    if (action === "toggle-study") await toggleStudyMode();
...
document.addEventListener("mousedown", async (event) => {
  ...
  if (!event.target.closest(".question-card")) return;
  const direction = quizRules.mouseNavigationAction({
    button: event.button,
    targetIsInteractive: mouseTargetIsInteractive(event.target),
  });
  if (!direction) return;
  event.preventDefault();
  await navigateQuestion(direction);
});

document.addEventListener("contextmenu", (event) => {
  if (appState.view !== "quiz") return;
  if (!event.target.closest(".question-card")) return;
  if (mouseTargetIsInteractive(event.target)) return;
  event.preventDefault();
});
