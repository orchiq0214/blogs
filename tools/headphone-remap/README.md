...
```powershell
# 1. 克隆仓库（路径自定）
git clone <repo-url> C:\path\to\headphone-remap
cd C:\path\to\headphone-remap
...
**默认安装路径**：`%LOCALAPPDATA%\Tools\AutoHotkey\`（跨用户通用，不会写死任何用户名）。

**自定义路径**：`install.ps1 -InstallDir 'D:\MyTools\AHK'`

**其他参数**：
- `-SkipAutostart`：不配置开机自启
- `-OpenTestPage`：装完自动打开测试页
- `-Uninstall`：卸载（删自启 + 关掉 AHK）

### 手动部署

如果不想用 install.ps1，也可以手动：

```
<install-dir>/
├── AutoHotkey64.exe          ← 从 https://www.autohotkey.com/download/ahk-v2.zip 解压
├── headphone_remap.ahk       ← 复制 scripts/ 下
├── headphone_watcher.ps1     ← 复制 scripts/ 下（路径已用 $PSScriptRoot 自适应）
└── setup_autostart.ps1       ← 可选，配开机自启
```

### 验证

按耳机 3 个键，右上角应该弹出蓝色提示：
- `+` → 显示 `↑`，选项上移
- `−` → 显示 `↓`，选项下移
- 中键 → 显示 `Enter`，确认答案
...
| 日志 | 位置 | 内容 |
|---|---|---|
| 看门狗 | `<install-dir>\watcher_log.txt` | 插拔事件、AHK 启停 |
| 按键 | `<install-dir>\headphone_log.txt` | 每次按键触发 |

（`<install-dir>` 默认 `%LOCALAPPDATA%\Tools\AutoHotkey`，可改）
